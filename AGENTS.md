...
- Test suite: 93/93 pass (frontend), 22/22 pass (backend), typecheck clean.
...
- **Idle `doublePress` events on the G2 / simulator can mimic user input.** Both the G2 simulator (`@evenrealities/evenhub-simulator@0.7.2`) and the real G2 hardware (on screen-timeout) fire system `doublePress` events (eventType: 3, eventSource: 1) when nothing is happening. Each pair sends the controller to `asleep` and back, so the state oscillates without user action. On the simulator this happens every 4-9s; on real G2 it happens once per screen-timeout (~30s). The user perceives this as "scroll doesn't work", "I select something and something else opens up", or "I don't know which state I am in". **Fix at the controller level** by ignoring `doublePress` events that arrive when `eventSource === 1` and the last user input was more than 1s ago, or that are not accompanied by a fresh user `press`/`swipe` event in the same window. The harness has a step (`49-idle-no-asleep-bounce` in `docs/UI_INVARIANTS.json`) and a matcher (`noLifecycles: ["asleep", "wake"]` in `scripts/simulator-flow.mjs`) that catches this regression by asserting no `lifecycle: asleep` or `lifecycle: wake` events fire during a known-good interaction with a 4s+ budget.
## Pending Checklist
...
The harness now runs cleanly in real mode with exit code 0 (48/48 steps pass, 0 failures). Blank glasses-screenshots from the simulator API are replaced with full-screen macOS `screencapture` fallbacks that consistently produce visible output. The remaining golden-mismatch warnings are expected: full-screen captures have different dimensions than the simulator's 576×288 golden images.